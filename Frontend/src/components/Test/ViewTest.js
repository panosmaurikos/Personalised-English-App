...
function ViewTest({ test, onClose }) {
  ...
  return (
    <div className={styles.container}>
      ...
      <p className={styles.info}>
        <strong>Created:</strong> {new Date(test.created_at).toLocaleString()}
      </p>
      <p className={styles.info}>
        <strong>Updated:</strong> {new Date(test.updated_at).toLocaleString()}
      </p>
          ...
          {q.options && (
            <ul className={styles.optionsList}>
              {Object.entries(q.options).map(([key, value]) => (
                <li
                  key={key}
                  className={key === q.correct_answer ? styles.correct : ""}
                >
                  {key}: {value}
                </li>
              ))}
